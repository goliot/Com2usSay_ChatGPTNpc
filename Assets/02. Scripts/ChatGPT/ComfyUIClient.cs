...
public class ComfyUIClient : MonoBehaviour
{
    ...
    private string outputFolderPath = @"C:\ComfyUI\ComfyUI_windows_portable\ComfyUI\output";
    ...
    private void UpdatePromptInWorkflow(JObject workflow, string prompt)
    {
        try
        {
            ...
            {
                Debug.LogWarning("⚠️ 긍정 프롬프트 노드(4)를 찾을 수 없습니다.");
            }
        }
        catch (Exception e)
        {
    ...
    private void CheckBasicConnections(JObject workflow)
    {
        try
        {
            ...
            if (vaeNode?["inputs"] != null)
            {
                ...
                if (inputs["samples"] != null)
                    Debug.Log($"🔗 VAEDecode 샘플 연결: {inputs["samples"]}");
                if (inputs["vae"] != null)
        ...
        {
            Debug.LogWarning($"⚠️ 연결 상태 확인 중 오류: {e.Message}");
        }
    }
    ...
    private string GetLatestImageFile()
    {
        try
        {
            ...
            {
                Debug.LogWarning("⚠️ 출력 폴더에 이미지 파일이 없습니다.");
                return null;
            }
